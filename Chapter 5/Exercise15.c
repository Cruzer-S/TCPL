...
{
	int nlines, c;
	int numeric, reverse, cases;
	int (*comp)(const void *, const void*);

	cases = numeric = reverse = 0;
	...
	if (numeric) {
		...
		if (cases) comp = (int (*)(const void *, const void*))stricmp;
		else       comp = (int (*)(const void *, const void*))strcmp;
	}

	if ((nlines = readlines(lineptr, MAXLINES)) >= 0) {
		mqsort((void **) lineptr, 0, nlines - 1, reverse, comp);
